...
import org.apache.spark.serializer.{SerializerManager, JavaSerializer, KryoSerializer}
...
class BlockManagerSuite extends SparkFunSuite with Matchers with BeforeAndAfterEach
  ...
  private def makeBlockManager(
    ...
    val blockManager = new BlockManager(name, rpcEnv, master, new SerializerManager(serializer),
      conf, memManager, mapOutputTracker, shuffleManager, transfer, securityMgr, 0)
  ...
  test("block store put failure") {
    ...
    store = new BlockManager(SparkContext.DRIVER_IDENTIFIER, rpcEnv, master,
      new SerializerManager(new JavaSerializer(conf)), conf, memoryManager, mapOutputTracker,
  ...
  ignore("safely unroll blocks") {
    ...
    val unrollSafely = PrivateMethod[Either[Array[Any], Iterator[Any]]]('unrollSafely)

    var unrollResult = memoryStore invokePrivate unrollSafely("unroll", smallList.iterator)
    ...
    unrollResult = memoryStore invokePrivate unrollSafely("unroll", smallList.iterator)
    verifyUnroll(smallList.iterator, unrollResult, shouldBeArray = true)
    assert(memoryStore.currentUnrollMemoryForThisTask === 0)
    ...
    unrollResult = memoryStore invokePrivate unrollSafely("unroll", bigList.iterator)
